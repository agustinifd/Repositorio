...
function comentar()
{
  ...
  <div class="card-body">
    <blockquote class="blockquote mb-0">
      ...
      <footer class="blockquote-footer">${document.getElementById("profile").textContent}<cite title="Source Title"><br>Publicado:${time.getFullYear()}-${(time.getMonth()+1)}-${(time.getDay()-1)}<br>${time.getHours()}:${time.getMinutes()}:${time.getSeconds()}</cite></footer>
    </blockquote>
  </div>
</div>`;
}
